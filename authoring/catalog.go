...
func ManagementDomain() capabilitycontract.CapabilityAuthoringDomain {
	return capabilitycontract.CapabilityAuthoringDomain{Key: "scheduler", Capabilities: []capabilitycontract.CapabilityAuthoringDefinition{
		...
		schedulerCommandAuthoringCapability("scheduler.job.simulate", schedulersdk.ActionSchedulerDefinitionsSimulate, "business_schedule_simulation", "definition_id", "POST /scheduler/definitions/{definitionID}/simulate", false),
		schedulerCommandAuthoringCapability("scheduler.job.run", schedulersdk.ActionSchedulerDefinitionsRun, "business_schedule_execution", "definition_id", "POST /scheduler/definitions/{definitionID}/run", true),
		schedulerCommandAuthoringCapability("scheduler.run.retry", schedulersdk.ActionSchedulerRunsRetry, "business_administrator_recovery", "run_id", "POST /scheduler/runs/{runID}/retry", true),
		schedulerCommandAuthoringCapability("scheduler.run.cancel", schedulersdk.ActionSchedulerRunsCancel, "business_administrator_recovery", "run_id", "POST /scheduler/runs/{runID}/cancel", true),
...
func schedulerBusinessJobAuthoringCapability() capabilitycontract.CapabilityAuthoringDefinition {
	...
	return capabilitycontract.CapabilityAuthoringDefinition{
		...
		ValidationEndpoint: "POST /scheduler/definitions/validate", PreviewEndpoint: "POST /scheduler/definitions/validate",
		ConfigurationRoutes: []string{"GET /metadata/definitions/scheduler/{resourceKey}", "POST /metadata/definitions/scheduler/{resourceKey}/validate", "GET /authoring/snapshot", "GET /references", "POST /scheduler/definitions/validate", "GET /scheduler/definitions/{definitionID}"}, ResourceKeyPathParameter: "definitionID",
		...
		ReferenceContracts: []capabilitycontract.CapabilityAuthoringReference{{Kind: "scheduler_target_key", InputJSONPointer: "/target_key", ScopeFrom: "/target_type", ResolverEndpoint: "/discovery/references/scheduler_target_key"}, {Kind: "object_key", InputJSONPointer: "/target_object", ResolverEndpoint: "/discovery/references/object_key"}, {Kind: "role_key", InputJSONPointer: "/run_as_role", ResolverEndpoint: "/discovery/references/role_key"}, {Kind: "business_calendar_key", InputJSONPointer: "/business_calendar_key", ResolverEndpoint: "/discovery/references/business_calendar_key"}},
		Execution:          &capabilitycontract.CapabilityAuthoringExecution{ReadSet: []string{"metadata.scheduler_definition", "metadata.business_calendar", "action.definition", "workflow.definition", "report.definition", "identity.service_role"}, Transaction: "read_only_candidate_validation", Idempotency: "naturally_idempotent_at_candidate_hash", SideEffectLevel: "none", PermissionModel: exactActionPermissionModel, ChangeControl: "source_controlled_json"},
...
func schedulerResolveDeadLetterAuthoringCapability() capabilitycontract.CapabilityAuthoringDefinition {
	...
	capability := schedulerCommandAuthoringCapability("scheduler.dead_letter.resolve", schedulersdk.ActionSchedulerDeadLettersResolve, "business_administrator_recovery", "dead_letter_id", "POST /scheduler/dead-letters/{deadLetterID}/resolve", true)
